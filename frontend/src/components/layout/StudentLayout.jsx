      ...
      <header className="border-b border-slate-200 bg-white">
        ...
        <nav className={`${menuOpen ? 'block' : 'hidden'} border-t border-slate-200 px-4 py-3 md:block md:border-0 md:px-6 md:py-0`}>
          <div className="mx-auto flex max-w-7xl flex-col gap-1 md:flex-row md:gap-6">
            {links.map((link) => <NavLink key={link.to} className={({ isActive }) => `rounded px-2 py-2 text-sm font-medium ${isActive ? 'bg-blue-50 text-blue-700' : 'text-slate-600 hover:text-slate-900'}`} onClick={() => setMenuOpen(false)} to={link.to}>{link.label}</NavLink>)}
            <div className="flex items-center justify-between border-t border-slate-200 py-2 md:hidden"><span className="text-sm text-slate-600">{user?.name}</span><button className="text-sm text-slate-700" onClick={handleLogout} type="button">Log out</button></div>
          </div>
        </nav>
      </header>
